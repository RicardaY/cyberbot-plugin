...
const openai = new OpenAI({
    baseURL: 'https://api.deepseek.com', // 设置 API 地址
    apiKey: '<DeepSeek API Key>' // 设置 API 密钥
});
// md2html地址
...
async function deepseek(msg :string) {
    const completion = await openai.chat.completions.create({
      messages: [
        { role: "system", content: "请你扮演一个被安排在 QQ 群中的全能问答机器人，你拥有海量的知识储备，可以回答各种问题，且每次回答都不少于 100 个字符，你的回答生动而形象，回复内容当中的很多 emoji 的运用得体而不胡哨，所有回复将被渲染成漂亮好看的 Markdown，你尽可能的使用 markdown 强大的语法和生动形象的语言来呈现你的回答，除非群友特殊说明，请尽可能使用中文回复，接下来请你回复或解答以下群友的问题，必要时可以用生动形象的语言，也可以带一点幽默的群友搞怪口吻。请直接回复下列内容：" },
